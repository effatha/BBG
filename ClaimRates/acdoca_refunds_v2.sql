/*******************************************
**
[WR].[WR_TX_L0_S4HANA_2LIS_03_BF_L1_FACT_A_STOCK_MOVEMENT]
[WR].[WR_SP_SRG_L1_FACT_A_STOCK_MOVEMENT]
[WR].[WR_HIST_FULL_L1_FACT_A_STOCK_MOVEMENT]

[WR_FIN].[WR_TX_L0_S4HANA_ACDOCA_L1_FACT_A_GENERAL_LEDGER]
[WR_FIN].[WR_SP_SRG_L1_FACT_A_GENERAL_LEDGER]
[WR_FIN].[WR_HIST_FULL_L1_FACT_A_GENERAL_LEDGER]
*******************************************/

--CREATE TABLE TEST.L1_FACT_A_CLAIM_RATES

--(
--	ProcessId 						 NVARCHAR(50)
--	,ProcessIdLine 					NVARCHAR(50)
--	,DocumentNo						 NVARCHAR(50)
--	,DocumentType 					 NVARCHAR(50)
--	,DocumentLine					 NVARCHAR(50)
--	,FIDocumentNo					 NVARCHAR(50)
--	,FIDocumentLine					 NVARCHAR(50)
--	,RefDocumentNo					 NVARCHAR(50)
--	,RefDocumentLine				 NVARCHAR(50)
...
--	,GrossOrderValue					DECIMAL(19,6)
--	,ItemQty							DECIMAL(19,6)
--	,RefundValue 					DECIMAL(19,6)
--	,RefundQty 						DECIMAL(19,6)
--	,ReplacementQty 					DECIMAL(19,6)
--	,ReturnQty 						DECIMAL(19,6)
--	,IsFullRefund 					 CHAR(1)
--	,IsExcessRefund					 CHAR(1)
...
FROM L1_FIN.L1_FACT_A_GENERAL_LEDGER fact
	...
	WHERE 1=1
		...
		--AND sales.CD_SALES_PROCESS_ID = '0405287484'
		--	and CD_FI_DOCUMENT_NO = '6302296916'
	GROUP BY
		sales.CD_DOCUMENT_NO,
		--CD_FI_DOCUMENT_LINE,
		fact.ID_ITEM,
...
SELECT
	...
	ReturnQty =	ReturnQTY,
--	Test = CASE WHEN SUM([VL_REFUNDS_QTY]) over(partition by ProcessId,process.ProcessIDPosition) <> 0 THEN  SUM([VL_REFUNDS_QTY]) over(partition by ProcessId,process.ProcessIDPosition) ELSE 0 END,
...
--	TotalRefundValue = SUM([AMT_REFUNDS_EUR]) over(partition by ProcessId,process.ProcessIDPosition),
--	TotalGrossOrderValue = SUM(GrossOrderValue) over(partition by ProcessId,process.ProcessIDPosition)
...
--000100
	...
	--select MONTH(PostingDate)OrderMonth, SUM(RefundValue)RefundValue
	--from TEST.L1_FACT_A_CLAIM_RATES
	--WHERE 
	--	YEAR(PostingDate) = 2024
	--GROUP BY MONTH(PostingDate)


	--select MONTH(PostingDate)OrderMonth,sum(GrossOrderValue) GOV, SUM(RefundValue)RefundValue, (SUM(RefundValue)/sum(GrossOrderValue))
	--from TEST.L1_FACT_A_CLAIM_RATES
	--WHERE 
	--	YEAR(PostingDate) = 2024
	--GROUP BY MONTH(PostingDate)
	...
	--select MONTH(DocumentDate)OrderMonth,sum(GrossOrderValue) GOV, SUM(RefundValue)RefundValue, (SUM(RefundValue)/sum(GrossOrderValue))
	--from TEST.L1_FACT_A_CLAIM_RATES_BCK
	--WHERE 
	--	YEAR(DocumentDate) = 2024
...
--	select * from [L1_FIN].[L1_DIM_A_FINANCE_TRAN_KPI_MATRIX] where VL_NET_PRODUCT_COSTS_THEREOF_REPLACEMENT_MEK_PARAM is not null



--select * from [L1_FIN].[L1_DIM_A_FINANCE_TRAN_KPI_MATRIX] fkart


select * from 	TEST.L1_FACT_A_CLAIM_RATES where processid= '0405723165'
 ---normal refund 0405259730
 --- order wihyt multiple line s for the same item: 0402293746
  --- order with set's: 0402293746

 select * from TEST.L1_FACT_A_CLAIM_RATES where ProcessID = '0402222686'