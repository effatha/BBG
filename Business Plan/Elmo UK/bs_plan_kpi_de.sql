--TRUNCATE TABLE [TEST].[L1_FACT_F_BUSINESS_PLAN_KPI_ELMO]

	DECLARE @GB_HandlingInboundPerUnit AS DECIMAL(19,4)
	DECLARE @GB_HandlingReturnsPerUnit AS DECIMAL(19,4)
	DECLARE @GB_WarehouseFixedCostsPercRevenue AS DECIMAL(19,4)
	DECLARE @GB_TruckingshipmentM3 AS DECIMAL(19,4)
	DECLARE @GB_HandlingTruckingshipmentUnit AS DECIMAL(19,4)

	SET @GB_HandlingInboundPerUnit = 1.86
	SET @GB_HandlingReturnsPerUnit = 4.46
	SET @GB_WarehouseFixedCostsPercRevenue = 0.03
	SET @GB_TruckingshipmentM3 = 28.6
	SET @GB_HandlingTruckingshipmentUnit = 0.65
...
CTE_SALES_L1 AS (
			...
			,AMT_HANDLING_RETURNS_COST_EUR = CASE WHEN ISNULL(fact.VL_ITEM_QUANTITY,0)=0 THEN 0 ELSE ISNULL(returns.[AMT_HANDLING_RETURNS_COST_EUR],0) END
			...
			*(CASE WHEN fact.[CD_COUNTRY_GROUP]<>'DE' and item.T_PRODUCT_HIERARCHY_3 in ('Cross Trainer', 'Rowing Machine', 'Sixpack Training', 'Cardio Bikes') THEN 1 ELSE item.VL_GROSS_WEIGHT*0.7/1000 END)*rest_wee.AMT_FEE_PER_WEIGHT * rate_wee.VL_FX_RATE  
			...
			,FL_EOL = CASE WHEN ISNULL(itemBS.CD_ITEM_STATUS_BUSINESS_PLAN,'') = 'EOL'
								OR ISNULL(item.CD_STATUS_PLANT,'') = 'Z5'
								OR ISNULL(item.[T_MI_ITEM_STATUS],'') = '-1'
								THEN 'Y' ELSE 'N' END
			,ItemVolumeM3 = CASE WHEN item.VL_VOLUME > 0 THEN item.VL_VOLUME / CASE WHEN item.cd_unit_volume='CCM' THEN 1000000 WHEN item.cd_unit_volume='M3' THEN 1 END ELSE 0 END

			FROM [TEST].[L1_FACT_F_BUSINESS_PLAN_ELMO] fact 
			LEFT JOIN [L1].[L1_DIM_A_ITEM] item 
...
,
	...
	(
		SELECT sales.*
				...
				,AMT_MARKETING_MARKETPLACES_EST_EUR = 0
	...
	(
		SELECT sales.*
				,GBHandlingInboundEst = CASE WHEN T_REVISED_LOCATION = 'Lager UK' THEN (VL_NET_ORDER_QUANTITY+VL_REPLACEMENT_ORDER_QUANTITY_EST)  * @GB_HandlingInboundPerUnit ELSE 0 END
				,GBHandlingReturnsEst = CASE WHEN T_REVISED_LOCATION = 'Lager UK' THEN (VL_RETURNED_QUANTITY_EST)  * @GB_HandlingReturnsPerUnit ELSE 0 END
				,GBTruckingTransShipmentEst = CASE WHEN T_REVISED_LOCATION = 'Lager UK' THEN (ItemVolumeM3 * VL_NET_ORDER_QUANTITY)  * @GB_TruckingshipmentM3 ELSE 0 END
				,GBHandlingTransShippmentEst = CASE WHEN T_REVISED_LOCATION = 'Lager UK' THEN (VL_NET_ORDER_QUANTITY)  * @GB_HandlingTruckingshipmentUnit ELSE 0 END
				,GBWarehousingRentEst = CASE WHEN T_REVISED_LOCATION = 'Lager UK' THEN (AMT_REVENUE_EST_EUR)  * @GB_WarehouseFixedCostsPercRevenue ELSE 0 END
	...
	(
		SELECT sales.*
						,AMT_PC2_EST_EUR = CASE WHEN NUM_ITEM NOT LIKE '9%' THEN
											...
											- ISNULL(GBHandlingInboundEst,0)
										- ISNULL(GBHandlingReturnsEst,0)
										- ISNULL(GBTruckingTransShipmentEst,0)
										- ISNULL(GBHandlingTransShippmentEst,0)
	...
	(
		SELECT sales.*
				 ...
				 (AMT_PC2_EST_EUR - ISNULL(AMT_WAREHOUSING_RENT_EST_EUR,0) - ISNULL(AMT_WAREHOUSING_OPEX_EST_EUR,0) - ISNULL(AMT_CS_MANAGEMENT_EST_EUR,0)
						- ISNULL(AMT_MARKETING_FIXED_COST_EST_EUR,0)  - ISNULL(GBWarehousingRentEst,0)
					)   ELSE AMT_PC1_EST_EUR END		
		FROM CTE_SALES_L10 sales
	)
INSERT INTO [TEST].[L1_FACT_F_BUSINESS_PLAN_KPI_ELMO]
(
	 D_SNAPSHOT
	 ,CD_SOURCE
	,ID_COMPANY
...
SELECT
	...
	,AMT_PACKAGING_EUR
	,AMT_HANDLING_ORDERS_EST_EUR
	,ISNULL(AMT_HANDLING_INBOUND_EST_EUR,0) + ISNULL(GBHandlingInboundEst,0)
	,ISNULL(AMT_HANDLING_TRANS_SHIPPMENT_EST_EUR,0)   + ISNULL(GBHandlingTransShippmentEst,0)
	,ISNULL(AMT_TRUCKING_TRANS_SHIPMENT_EST_EUR,0) + ISNULL(GBTruckingTransShipmentEst,0)
	,ISNULL(AMT_HANDLING_RETURNS_EST_EUR,0) +ISNULL(GBHandlingReturnsEst,0) 
	,AMT_WAREHOUSING_FBA_EUR
	,AMT_FULFILLMENT_OUTBOUND_EST_EUR
	,AMT_COMMISSIONS_MARKETPLACES_EUR
	,AMT_COMMISSIONS_MARKETPLACES_REFUNDS_EUR
	...
	,AMT_ENVIRO_AND_LICENSE_COST_EST_EUR
	,AMT_PC2_EST_EUR
	,ISNULL(AMT_WAREHOUSING_RENT_EST_EUR,0) + ISNULL(GBWarehousingRentEst,0)
	,AMT_WAREHOUSING_OPEX_EST_EUR
	,AMT_CS_MANAGEMENT_EST_EUR
	,AMT_MARKETING_FIXED_COST_EST_EUR
	,AMT_NET_ORDER_CONTRIBUTION_EST_EUR
	,AMT_PC3_EST_EUR = CASE
							WHEN FL_EOL = 'Y' THEN AMT_PC3_EST_EUR
							WHEN FL_EOL = 'N' and ISNULL(AMT_PC3_EST_EUR,0) < 0 THEN 0
							ELSE AMT_PC3_EST_EUR END	--- final
	,GETDATE()
	,GETDATE()
FROM CTE_SALES_L11
...
--select * from pl.pl_v_item where itemno = '10040451'

--SELECT 
--	AMT_PC2_EST_EUR,AMT_CS_MANAGEMENT_EST_EUR, AMT_CS_MANAGEMENT_EST_EUR,AMT_WAREHOUSING_OPEX_EST_EUR,AMT_WAREHOUSING_RENT_EST_EUR,AMT_PC3_EST_EUR,
--	AMT_PC3_EST_CALC = (AMT_PC2_EST_EUR-AMT_CS_MANAGEMENT_EST_EUR-AMT_WAREHOUSING_OPEX_EST_EUR-AMT_WAREHOUSING_RENT_EST_EUR-AMT_MARKETING_FIXED_COST_EST_EUR)
--from 
--[TEST].[L1_FACT_F_BUSINESS_PLAN_KPI_ELMO]
--where
--	id_item = 28711
--	and
--	cd_channel_group_3 = 'ShopWE'