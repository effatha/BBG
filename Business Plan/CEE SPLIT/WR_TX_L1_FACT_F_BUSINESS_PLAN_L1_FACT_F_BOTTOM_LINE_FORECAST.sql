/****** Object:  StoredProcedure [WR].[WR_TX_L1_FACT_F_BUSINESS_PLAN_L1_FACT_F_BOTTOM_LINE_FORECAST]    Script Date: 12/03/2025 10:57:48 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROC [TEST].[WR_TX_L1_FACT_F_BUSINESS_PLAN_L1_FACT_F_BOTTOM_LINE_FORECAST] AS
BEGIN

DELETE FROM  [TEST].[L1_FACT_F_BOTTOM_LINE_FORECAST]
WHERE month(D_TARGET) = month(getdate())
AND year(D_TARGET) =year(getdate())
...
INSERT INTO [TEST].[L1_FACT_F_BOTTOM_LINE_FORECAST]
(
...
SELECT                                    
      ...
      ,[VL_ITEM_QUANTITY] = (b_plan.VL_ITEM_QUANTITY/(1-FCFactor)) *  ISNULL(CAST(run_rates.rr as decimal(19,9)) ,1)
      ,[AMT_TARGET_NET_ORDER_VALUE_EUR] =(b_plan.AMT_TARGET_NET_ORDER_VALUE_EUR /(1-FCFactor)) *  ISNULL(CAST(run_rates.rr as decimal(19,9)) ,1)
