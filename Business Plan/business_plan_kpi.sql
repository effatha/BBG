-- SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_REPLACEMENT_PROXY_VALUES
...
--FORECAST 24
-- WR.WR_TX_L0_MI_FC_PLAN_L1_FACT_F_BUSINESS_PLAN

--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_FC_2024_PLAN_AMAZON
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_FC_2024_PLAN_B2B
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_FC_2024_PLAN_CEE
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_FC_2024_PLAN_MARKETPLACES
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_FC_2024_PLAN_SHOP

--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_BUSINESS_PLAN_AMAZON
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_BUSINESS_PLAN_B2B
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_BUSINESS_PLAN_CEE
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_BUSINESS_PLAN_MARKETPLACES
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_BUSINESS_PLAN_SHOP
--SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_BUSINESS_PLAN_ITEM

-- SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_RETURN_PROXY_VALUES
...
-- SELECT MAX(LOAD_TIMESTAMP) FROM L0.L0_MI_FULFILLMENT_SHIPPING_FACTOR
--[WR].[WR_TX_L0_MI_FULFILLMENT_SHIPPING_FACTOR_L1_DIM_A_FULFILLMENT_SHIPPING_FACTOR]
--WR.WR_SP_SRG_L1_DIM_A_FULFILLMENT_SHIPPING_FACTOR
--WR.WR_HIST_FULL_L1_DIM_A_FULFILLMENT_SHIPPING_FACTOR


--UPDATE  [L1].[L1_DIM_A_SALES_CHANNEL]  SET [CD_CHANNEL_GROUP_3] = 'ShopWE' WHERE [CD_CHANNEL_GROUP_3] IS NULL AND [CD_CHANNEL_GROUP_1] = 'Shop WE'
--UPDATE  [L1].[L1_DIM_A_SALES_CHANNEL]  SET [CD_CHANNEL_GROUP_3] = 'B2B' WHERE [CD_CHANNEL_GROUP_3] IS NULL AND [CD_CHANNEL_GROUP_1] = 'B2B'
--UPDATE  [L1].[L1_DIM_A_SALES_CHANNEL]  SET [CD_CHANNEL_GROUP_3] = 'CEE' WHERE [CD_CHANNEL_GROUP_3] IS NULL AND [CD_CHANNEL_GROUP_1] like '%CEE%'
--UPDATE  [L1].[L1_DIM_A_SALES_CHANNEL]  SET [CD_CHANNEL_GROUP_3] = 'MarketplacesWE' WHERE [CD_CHANNEL_GROUP_3] IS NULL AND [CD_CHANNEL_GROUP_1] like '%Marketplaces WE%'


--DECLARE @source as nvarchar(50) = 'ALL'
DECLARE @source as nvarchar(50) = 'BUD'
--DECLARE @source as nvarchar(50) = 'FC24'

IF(@source = 'ALL')
BEGIN
	TRUNCATE TABLE  L1.[L1_FACT_F_BUSINESS_PLAN_KPI] 
END
ELSE
BEGIN
	DELETE FROM L1.[L1_FACT_F_BUSINESS_PLAN_KPI] where CD_SOURCE = @source
END
...
--CTE_MARKETING_MAKETPLACES AS (
--	select D_VALID_FROM,D_VALID_TO, ch.CD_CHANNEL_GROUP_3,T_PRODUCT_HIERARCHY_1,T_PRODUCT_HIERARCHY_2,AVG(PCT_MARKETING)PCT_MARKETING
--	from
--	[L1].[L1_DIM_A_MARKETING_MARKETPLACES] mk
--	INNER JOIN L1.L1_DIM_A_SALES_CHANNEL ch
--			on ch.ID_SALES_CHANNEL = mk.ID_SALES_CHANNEL
--	GROUP BY D_VALID_FROM,D_VALID_TO,T_PRODUCT_HIERARCHY_1,T_PRODUCT_HIERARCHY_2,ch.CD_CHANNEL_GROUP_3

--),
--CTE_COMMISSIONS_MARKETPLACES AS
--(
--    select 
--		D_VALID_FROM,D_VALID_TO,
--		REPLACE(CASE WHEN [CD_CHANNEL_GROUP_1] = 'Marketplaces CEE' THEN 'CEE' ELSE [CD_CHANNEL_GROUP_1] END,' ','')[CD_CHANNEL_GROUP_3],T_PRODUCT_HIERARCHY_1,T_PRODUCT_HIERARCHY_2,avg(PCT_COMMISSIONS_ORDER) PCT_COMMISSIONS_ORDER,avg(PCT_COMMISSIONS_RETURN) PCT_COMMISSIONS_RETURN,avg(PCT_COMMISSIONS_MIN) PCT_COMMISSIONS_MIN
--	from 
--	[L1].[L1_DIM_A_CS_COMMISSIONS_MARKETPLACES]
--	GROUP BY D_VALID_FROM,D_VALID_TO,
--	REPLACE(CASE WHEN [CD_CHANNEL_GROUP_1] = 'Marketplaces CEE' THEN 'CEE' ELSE [CD_CHANNEL_GROUP_1] END,' ',''),T_PRODUCT_HIERARCHY_1,T_PRODUCT_HIERARCHY_2
...
CTE_TOTAL_VOLUME AS (
			SELECT 
				...
				--CAST(SUM(VL_ITEM_QUANTITY*item.VL_VOLUME) as decimal(19,8)) VL_TOTAL_VOLUME
...
--),
--CTE_FIXED_COSTS AS (
--select validfrom,validto, sum(warehouse_costs)warehouse_costs
--from L0.L0_MI_BUSINESS_PLAN_WAREHOUSE_FIXED_COSTS
...
CTE_SALES_L1 AS (
			...
			,fact.D_TARGET 													--AS TransactionDate
			,fact.ID_ITEM														--AS ItemID
			,fact.ID_ITEM_BUSINESS_PLAN												--AS ChannelId
			,fact.CD_FULFILLMENT												--AS Fulfillment
			,fact.T_REVISED_LOCATION											--AS StorageLocation
			,fact.[CD_COUNTRY_GROUP]												--AS DeliveryCountry
			,fact.[CD_CHANNEL_GROUP_3]
			,fact.VL_ITEM_QUANTITY													--AS Quantity
			,AMT_PLAN_PRICE_EUR													--AS NetPriceForeignCurrency
			...
			-- PC1 kpis
			--,AMT_DEMURRAGE_DETENTION_EUR =  1 *(ISNULL(costs.AMT_DEMURRAGE_DETENTION_EUR  / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )					
			--,AMT_DEADFREIGHT_EUR =  -1 *(ISNULL(costs.AMT_DEADFREIGHT_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) 	)						
			--,AMT_KICKBACKS_EUR= -1 *(ISNULL(costs.AMT_KICKBACKS_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )										
			--,AMT_3RD_PARTY_SERVICES_EUR= -1 *(ISNULL(costs.AMT_3RD_PARTY_SERVICES_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0)  )
			--,AMT_RETURN_MERCHANDISE_AUTHORIZATION_EUR =  -1 *(ISNULL(costs.AMT_RETURN_MERCHANDISE_AUTHORIZATION_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0)  )
			--,AMT_SAMPLES_EUR= -1 *(ISNULL(costs.AMT_SAMPLES_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )
			--,AMT_OTHER_COGS_EFFECT_EST_EUR= -1 *(ISNULL(costs.AMT_OTHER_COGS_EFFECT_EST_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )
			--,AMT_DROPSHIPMENT_CEOTRA9ER_EST_EUR =  -1 *(ISNULL(costs.AMT_DROPSHIPMENT_CEOTRA9ER_EST_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )
			--,AMT_INBOUND_FREIGHT_COST_EST_EUR =  -1 *(ISNULL(costs.AMT_INBOUND_FREIGHT_COST_EST_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0)  )		
			--,AMT_PO_CANCELLATION_EUR =  -1 *(ISNULL(costs.AMT_PO_CANCELLATION_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )				
			--,AMT_STOCK_ADJUSTMENT_EUR =  -1 *(ISNULL(costs.AMT_STOCK_ADJUSTMENT_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0)) 							
			--,AMT_FX_HEDGING_IMPACT_EST_EUR = -1 * (ISNULL(costs.AMT_FX_HEDGING_IMPACT_EST_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )
			--,AMT_COGS_STOCK_VALUE_ADJUSTMENT_EST_EUR =  -1 * (ISNULL(costs.AMT_COGS_STOCK_VALUE_ADJUSTMENT_EST_EUR / dvol.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )
...
--			,AMT_WAREHOUSE_FIXED_COSTS_EUR = (ISNULL(war_fixed_costs.WAREHOUSE_COSTS  / dvol_channel.VL_TOTAL_VOLUME * (fact.VL_ITEM_QUANTITY * item.VL_VOLUME),0) )					
		...
		--	AND 	fact.T_REVISED_LOCATION = packcost_min.T_STORAGE_LOCATION
		----	AND DATEADD(YEAR,-1,fact.D_TARGET_LOOKUP) BETWEEN packcost_min.D_VALID_FROM AND packcost_min.D_VALID_TO
		--	AND fact.D_TARGET BETWEEN packcost_min.D_VALID_FROM AND packcost_min.D_VALID_TO
		...
		LEFT JOIN [L1].[L1_DIM_A_TRUCKING_TRANS_SHIPMENTS] truckingtransship
			...
			--AND fact.D_TARGET BETWEEN truckingtransship.D_VALID_FROM AND truckingtransship.D_VALID_TO
	  --  LEFT JOIN CTE_MARKETING_MAKETPLACES marketplaces
			--ON REPLACE(fact.CD_CHANNEL_GROUP_3,' ','') = REPLACE(marketplaces.CD_CHANNEL_GROUP_3,' ','')
			--AND item.[T_PRODUCT_HIERARCHY_1] = marketplaces.[T_PRODUCT_HIERARCHY_1]
			--AND item.[T_PRODUCT_HIERARCHY_2] = marketplaces.[T_PRODUCT_HIERARCHY_2]
			--AND fact.D_TARGET_LOOKUP BETWEEN marketplaces.D_VALID_FROM AND marketplaces.D_VALID_TO
		--LEFT JOIN CTE_COMMISSIONS_MARKETPLACES			AS CM
		--	ON REPLACE(fact.CD_CHANNEL_GROUP_3,' ','') = CM.CD_CHANNEL_GROUP_3
		--	AND item.T_PRODUCT_HIERARCHY_1 = CM.T_PRODUCT_HIERARCHY_1
		--	AND item.T_PRODUCT_HIERARCHY_2 = CM.T_PRODUCT_HIERARCHY_2
		--	AND fact.D_TARGET_LOOKUP BETWEEN CM.D_VALID_FROM AND CM.D_VALID_TO
		...
		--ON DATEADD(YEAR,-1,fact.D_TARGET_LOOKUP) BETWEEN depreciation.D_VALID_FROM AND depreciation.D_VALID_TO
		--	AND fact.T_REVISED_LOCATION=depreciation.[T_STORAGE_LOCATION]
		...
		--LEFT JOIN CTE_FIXED_COSTS war_fixed_costs
		--	on 
		--		 fact.D_TARGET_LOOKUP BETWEEN war_fixed_costs.VALIDFROM AND war_fixed_costs.VALIDTO 
		--LEFT JOIN CTE_TOTAL_VOLUME_CHANNEL dvol_channel
		--		on dvol_channel.NUM_TARGET_MONTH = MONTH(D_TARGET)
		--		  AND dvol_channel.NUM_TARGET_YEAR = YEAR(D_TARGET)
		--		--  AND REPLACE(dvol_channel.CD_CHANNEL_GROUP_3,' ','')= fact.[CD_CHANNEL_GROUP_3]
	...
	--	(D_MONTH,CD_CHANNEL_GROUP_3,com_mkt_bs.VL_MARKETPLACE_RATE)
		WHERE
			 CD_SOURCE = @source OR @source = 'ALL'
)		
,
	CTE_SALES_L2 AS 
	(
		SELECT sales.*
			...
			--,AMT_COMMISSIONS_MARKETPLACES_EUR  = CASE WHEN CD_CHANNEL_GROUP_3 = 'CEE' then 0 ELSE ([AMT_TARGET_NET_ORDER_VALUE_EUR] * [PCT_COMMISSIONS_MARKETPLACES]) END
			...
			--,AMT_COGS_OPERATIONS_EST = (AMT_DEMURRAGE_DETENTION_EUR+AMT_DEADFREIGHT_EUR+AMT_KICKBACKS_EUR+AMT_3RD_PARTY_SERVICES_EUR+AMT_RETURN_MERCHANDISE_AUTHORIZATION_EUR
			--							+AMT_SAMPLES_EUR+AMT_OTHER_COGS_EFFECT_EST_EUR+AMT_DROPSHIPMENT_CEOTRA9ER_EST_EUR+AMT_INBOUND_FREIGHT_COST_EST_EUR
			--							+AMT_PO_CANCELLATION_EUR+AMT_STOCK_ADJUSTMENT_EUR)
...
,
	...
	(
		SELECT sales.*
			...
			--	 ,AMT_WAREHOUSING_RENT_EST_EUR = WarehousingRentEst_PreCalc --AMT_WAREHOUSE_FIXED_COSTS_EUR /2								
			--	 ,AMT_WAREHOUSING_OPEX_EST_EUR = --AMT_WAREHOUSE_FIXED_COSTS_EUR /2		
	...
	(
		SELECT sales.*
			...
			--	,VL_NET_ORDER_QUANTITY_EST = (VL_NET_ORDER_QUANTITY - VL_REFUNDED_QUANTITY_EST) 
	...
	(
		SELECT sales.*
				...
				-- ,AMT_PC1_EST_EUR = (AMT_PC0_EST_EUR - AMT_FX_HEDGING_IMPACT_EST_EUR - AMT_COGS_STOCK_VALUE_ADJUSTMENT_EST_EUR - AMT_COGS_OPERATIONS_EST) 
...
INSERT INTO L1.[L1_FACT_F_BUSINESS_PLAN_KPI]
(
	 D_SNAPSHOT
	 ,CD_SOURCE
	,ID_COMPANY
	...
	--,AMT_RETURN_MERCHANDISE_AUTHORIZATION_EUR
	--,AMT_SAMPLES_EUR
	--,AMT_OTHER_COGS_EFFECT_EST_EUR
	--,AMT_DROPSHIPMENT_CEOTRA9ER_EST_EUR
...
--	END
	...
	--SELECT sum(AMT_WAREHOUSING_RENT_EST_EUR)AMT_WAREHOUSING_RENT_EST_EUR,sum(AMT_WAREHOUSING_OPEX_EST_EUR)AMT_WAREHOUSING_OPEX_EST_EUR  
	--FROM  [TEST].[L1_FACT_F_BUSINESS_PLAN_KPI]
		
		

		--SELECT T_REVISED_LOCATION,SUM(isnull(VL_ITEM_QUANTITY,0) * isnull(VL_VOLUME,0)),
		
		--		SUM(isnull(VL_ITEM_QUANTITY,0) * (isnull(VL_VOLUME,0) / (CASE WHEN it.cd_unit_volume='CCM' THEN 1000000 WHEN it.cd_unit_volume='M3' THEN 1 END)))
		--		,TOTAL_COSTS = sum(isnull(AMT_WAREHOUSING_FBA_EUR,0))
		--		+sum(isnull(AMT_HANDLING_INBOUND_EST_EUR,0))  
		--		+sum(isnull(AMT_HANDLING_TRANS_SHIPPMENT_EST_EUR,0))  
		--		+sum(isnull(AMT_HANDLING_RETURNS_EST_EUR,0))  
		--		+sum(isnull(AMT_HANDLING_ORDERS_EST_EUR,0))  
		--		,AMT_WAREHOUSING_FBA_EUR = sum(isnull(AMT_WAREHOUSING_FBA_EUR,0))
		--		,AMT_HANDLING_INBOUND_EST_EUR = sum(isnull(AMT_HANDLING_INBOUND_EST_EUR,0)) 
		--		,AMT_HANDLING_TRANS_SHIPPMENT_EST_EUR = sum(isnull(AMT_HANDLING_TRANS_SHIPPMENT_EST_EUR,0))  
		--		,AMT_HANDLING_RETURNS_EST_EUR = sum(isnull(AMT_HANDLING_RETURNS_EST_EUR,0))  
		--		,AMT_HANDLING_ORDERS_EST_EUR = sum(isnull(AMT_HANDLING_ORDERS_EST_EUR,0))  
		--		,AMT_WAREHOUSING_FBA_EUR = sum(isnull(AMT_WAREHOUSING_FBA_EUR,0))
		--FROM  [TEST].[L1_FACT_F_BUSINESS_PLAN_KPI] kpi
		--LEFT JOIN l1.L1_DIM_A_ITEM it on it.ID_ITEM = kpi.ID_ITEM
			
		--Group by T_REVISED_LOCATION 
	...
	--SELECT sum(AMT_WAREHOUSING_RENT_EST_EUR)AMT_WAREHOUSING_RENT_EST_EUR,sum(AMT_WAREHOUSING_OPEX_EST_EUR)AMT_WAREHOUSING_OPEX_EST_EUR  
	--FROM  [l1].[L1_FACT_F_BUSINESS_PLAN_KPI]
	
	--ALTER TABLE L1.L1_FACT_F_BUSINESS_PLAN_KPI ADD AMT_NET_ORDER_CONTRIBUTION_EST_EUR							DECIMAL(19,2)
...
--	 select * into [L1].[L1_FACT_F_BUSINESS_PLAN_KPI_bck20241016] from  [L1].[L1_FACT_F_BUSINESS_PLAN_KPI]
--   select * into [L1].[L1_FACT_F_BUSINESS_PLAN_bck20241014] from  [L1].[L1_FACT_F_BUSINESS_PLAN]



--ALTER TABLE [L0].[L0_MI_BUSINESS_PLAN_ITEM] ADD ELASTICITYRISKBUCKET varchar(50)


 --select * into [L1].[L1_FACT_F_BUSINESS_PLAN_KPI] from  [TEST].[L1_FACT_F_BUSINESS_PLAN_KPI]




 --select top 10 VL_NET_ORDER_QUANTITY,AMT_MEK_HEDGING_EUR,AMT_GTS_MARKUP_EUR,AMT_NET_ORDER_VALUE_EST_EUR,AMT_NET_ORDER_CONTRIBUTION_EST_EUR,AMT_REVENUE_EST_EUR,AMT_NET_PRODUCT_COST_EST_EUR,AMT_PC0_EST_EUR
 --from [L1].[L1_FACT_F_BUSINESS_PLAN_KPI] 



 --update [TEST].[L1_FACT_F_BUSINESS_PLAN_KPI]  SET AMT_NET_ORDER_CONTRIBUTION_EST_EUR = AMT_NET_ORDER_CONTRIBUTION_EST_EUR - (AMT_GTS_MARKUP_EUR*VL_NET_ORDER_QUANTITY)


 --select * into [L1].[L1_FACT_F_BUSINESS_PLAN_KPI_SINA] from [L1].[L1_FACT_F_BUSINESS_PLAN_KPI] 
--select * into  [L1].[L1_FACT_F_BUSINESS_PLAN_SINA] FROM  [L1].[L1_FACT_F_BUSINESS_PLAN]

--update [L1].[L1_FACT_F_BUSINESS_PLAN_KPI]  SET AMT_NET_ORDER_CONTRIBUTION_EST_EUR = AMT_NET_ORDER_CONTRIBUTION_EST_EUR + (AMT_GTS_MARKUP_EUR*VL_NET_ORDER_QUANTITY)



--SELECT 
--	SUM(AMT_COMMISSIONS_MARKETPLACES_REFUNDS_EUR)AMT_COMMISSIONS_MARKETPLACES_REFUNDS_EUR,
--	SUM(AMT_COMMISSIONS_MARKETPLACES_EUR)AMT_COMMISSIONS_MARKETPLACES_EUR,
--	SUM(AMT_NET_ORDER_VALUE_EST_EUR)AMT_NET_ORDER_VALUE_EST_EUR,
--	NOV_WITH_VAT = SUM(AMT_NET_ORDER_VALUE_EST_EUR * 1.22) ,
--	PCT_COMMISSIONS = SUM(AMT_COMMISSIONS_MARKETPLACES_REFUNDS_EUR) / SUM(AMT_NET_ORDER_VALUE_EST_EUR * 1.22)

--FROM  [L1].[L1_FACT_F_BUSINESS_PLAN_KPI]
--WHERE D_TARGET>= '2025-01-01'
