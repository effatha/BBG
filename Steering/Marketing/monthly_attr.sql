...
(
	SELECT *,
		AMT_TOTAL_NOV_FAMILY_EUR = SUM(ISNULL(AMT_NET_ORDER_VALUE_EST_EUR,0)) 
					OVER(PARTITION BY T_PRODUCT_HIERARCHY_4,  DATEPART(MONTH, D_CREATED) ,[CD_CHANNEL_GROUP_3],CD_COUNTRY_GROUP_DELIVERY_SM )
		,PERC_NOV_FAMILY_SHARE = CASE WHEN AMT_NET_ORDER_VALUE_EST_EUR = 0 THEN 0 ELSE AMT_NET_ORDER_VALUE_EST_EUR / SUM(ISNULL(AMT_NET_ORDER_VALUE_EST_EUR,0)) 
					OVER(PARTITION BY T_PRODUCT_HIERARCHY_4, DATEPART(MONTH, D_CREATED) ,[CD_CHANNEL_GROUP_3],CD_COUNTRY_GROUP_DELIVERY_SM )END
		,AMT_TOTAL_NOV_ITEM_EUR = SUM(ISNULL(AMT_NET_ORDER_VALUE_EST_EUR,0)) 
					OVER(PARTITION BY NUM_ITEM, DATEPART(MONTH, D_CREATED) ,[CD_CHANNEL_GROUP_3],CD_COUNTRY_GROUP_DELIVERY_SM )
		,PERC_NOV_ITEM_SHARE = CASE WHEN AMT_NET_ORDER_VALUE_EST_EUR = 0 THEN 0 ELSE AMT_NET_ORDER_VALUE_EST_EUR / SUM(ISNULL(AMT_NET_ORDER_VALUE_EST_EUR,0)) 
					OVER(PARTITION BY NUM_ITEM, DATEPART(MONTH, D_CREATED) ,[CD_CHANNEL_GROUP_3],CD_COUNTRY_GROUP_DELIVERY_SM )END
...
(
	SELECT
		TransactionPeriod = DATEPART(MONTH, TransactionDate) ,
		ProductFamily,
		ChannelGroup3,
		CountryGroup,
...
(
	...
	LEFT JOIN cte_agg_mkt_fam mkt_fam 
			on  
				mkt_fam.TransactionPeriod = DATEPART(MONTH, D_CREATED)
				AND
	...
	LEFT JOIN cte_agg_mkt_item mkt_item 
			on  
				mkt_item.TransactionPeriod = DATEPART(MONTH, D_CREATED)
				AND
