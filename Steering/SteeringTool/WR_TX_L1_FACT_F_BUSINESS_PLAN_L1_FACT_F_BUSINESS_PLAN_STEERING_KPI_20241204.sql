/****** Object:  StoredProcedure [WR].[WR_TX_L1_FACT_F_BUSINESS_PLAN_L1_FACT_F_BUSINESS_PLAN_STEERING_KPI]    Script Date: 04/12/2024 17:09:10 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [WR].[WR_TX_L1_FACT_F_BUSINESS_PLAN_L1_FACT_F_BUSINESS_PLAN_STEERING_KPI] @source [nvarchar](50) AS
BEGIN
IF(@source IS NULL)
BEGIN
	SET @source= 'ALL'
...
IF(@source = 'ALL')
BEGIN
	TRUNCATE TABLE  L1.[L1_FACT_F_BUSINESS_PLAN_STEERING_KPI] 
END
ELSE
BEGIN
	DELETE FROM L1.[L1_FACT_F_BUSINESS_PLAN_STEERING_KPI] where CD_SOURCE = @source
END
...
CTE_SALES_L1 AS (
			...
			,fact.CD_CURRENCY													
			,item.NUM_ITEM
			,VL_REFUND_RATE = CASE WHEN ISNULL(ISNULL(fact.VL_REFUND_RATE,ref.VL_RATE),0.1) = 0 THEN 0.1 ELSE ISNULL(ISNULL(fact.VL_REFUND_RATE,ref.VL_RATE),0.1)END	
			,VL_RETURN_RATE = CASE WHEN ISNULL(ISNULL(fact.VL_RETURN_RATE,ret.VL_RATE),0.1)	= 0 THEN 0.1 ELSE ISNULL(ISNULL(fact.VL_RETURN_RATE,ret.VL_RATE),0.1)END											
			-- PC1 kpis
			...
			,VL_REPLACEMENTPRODUCTCOSTEST_PRECAL_EUR =  CASE WHEN ISNULL(ISNULL(fact.[VL_REPLACEMENT_RATE],replacement.VL_RATE),0)	= 0 THEN 0.1 ELSE ISNULL(ISNULL(fact.[VL_REPLACEMENT_RATE],replacement.VL_RATE),0) END
			,VL_REPLACEMENTORDERQUANTITYEST_PRECAL_EUR = CASE WHEN  ISNULL(ISNULL(fact.[VL_REPLACEMENT_RATE],replacement.VL_QUANTITY_RATE),0) = 0 THEN 0.1 ELSE ISNULL(ISNULL(fact.[VL_REPLACEMENT_RATE],replacement.VL_QUANTITY_RATE),0) END
			...
			*(CASE WHEN fact.[CD_COUNTRY_GROUP]<>'DE' and item.T_PRODUCT_HIERARCHY_3 in ('Cross Trainer', 'Rowing Machine', 'Sixpack Training', 'Cardio Bikes') THEN 1 ELSE item.VL_GROSS_WEIGHT*0.7/1000 END)*rest_wee.AMT_FEE_PER_WEIGHT * rate_wee.VL_FX_RATE  
			...
			FROM [L1].[L1_FACT_F_BUSINESS_PLAN_STEERING] fact 
			LEFT JOIN [L1].[L1_DIM_A_ITEM] item 
           ...
           LEFT JOIN L1.L1_DIM_A_SALES_REFUND_VALUES ref  
			...
			LEFT JOIN L1.[L1_DIM_A_DEPRECIATION_VALUES] depr  
				on fact.D_TARGET BETWEEN depr.D_VALID_FROM AND depr.D_VALID_TO
...
,
	...
	(
		SELECT sales.*
			...
			--	,VL_NET_ORDER_QUANTITY_EST = (VL_NET_ORDER_QUANTITY - VL_REFUNDED_QUANTITY_EST) 
				...
				END 
				,AMT_DEPRECIATION_EST_EUR = VL_RETURNED_QUANTITY_EST*AMT_DEPRECIATIONEST_PRECALC_EUR 
	...
	(
		SELECT sales.*
				 ...
				 ,AMT_PC0_EST_EUR = (AMT_REVENUE_EST_EUR - AMT_NET_PRODUCT_COST_EST_EUR - (VL_REPLACEMENTPRODUCTCOSTEST_PRECAL_EUR * AMT_NET_PRODUCT_COST_EST_EUR) - ISNULL(AMT_DEPRECIATION_EST_EUR,0))
...
INSERT INTO L1.[L1_FACT_F_BUSINESS_PLAN_STEERING_KPI]
(
	 D_SNAPSHOT
	 ,CD_SOURCE
	,ID_COMPANY
...
SELECT
	...
	,[VL_REFUND_RATE]
	,[VL_RETURN_RATE]
	,VL_REPLACEMENTPRODUCTCOSTEST_PRECAL_EUR
	---kpi--QTY
	,VL_NET_ORDER_QUANTITY
	,VL_REFUNDED_QUANTITY_EST
	,VL_RETURNED_QUANTITY_EST
