...
ALTER VIEW [PL].[PL_V_SALES_TRANSACTIONS_SM_NEW]
AS with cte_mkt_cost as (
	SELECT
		 ID_SALES_TRANSACTION
		,SUM(AMT_MKT_COST_EUR) AS AMT_MARKETING_COST_EUR
	FROM L1.L1_FACT_A_SALES_TRANSACTION_MKT_KPI_SM_NEW
	where 1=0
...
SELECT
	...
	,ISNULL(CD_COUNTRY_DELIVERY, CASE WHEN ch.CD_CHANNEL_GROUP_3 = 'CEE' THEN 'CEE' ELSE 'INT' END)		as  DeliveryCountry
...
WHERE
	...
	AND
	ISNULL([FL_INCIDENT],'N') = 'N'

UNION ALL

SELECT 
	 NULL							as  SalesTransactionCode
	,'Amazon'							AS	Source
	,NULL								as  ProcessId
	,NULL								as  CompanyId
	,D_CAMPAIGN							as  TransactionDate
	,ID_ITEM							as  ItemID
	,'A'								as  ItemType
	,ID_SALES_CHANNEL					as  ChannelId
	,NULL								as  Fulfillment
	,CD_COUNTRY							AS	DeliveryCountryGroup 
	,NULL								as  MarketplaceOrderId
	,NULL								as  PaymentMethod
	,NULL								as  StorageLocationCode
	,NULL								as  StorageLocation
	,CD_COUNTRY							as  DeliveryCountry
	,NULL 								AS  RefundRate
	,NULL								AS  ReturnRate
...
--FROM L1.L1_FACT_A_SALES_TRANSACTION_MKT_KPI_SM_NEW s
FROM PL.PL_V_MARKETING_COSTS_AMZ_SM s
