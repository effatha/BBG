DECLARE @nMonth as int  = 0
DECLARE @PRICE_DATE as DATE
SET @PRICE_DATE = '2024-08-01'
...
--SELECT DATEADD(month, DATEDIFF(month, 0, '2024-02-03'), 0) 



--select distinct D_EFFECTIVE from  [L1].[L1_FACT_F_PLAN_PRICE] order by 1



--DELETE
--FROM  [L1].[L1_FACT_F_PLAN_PRICE]
-- where D_EFFECTIVE > getdate()




--	update p SET D_EFFECTIVE= DATEADD(month, DATEDIFF(month, 0, D_EFFECTIVE), 0) 
--	from [L1].[L1_FACT_F_PLAN_PRICE] p