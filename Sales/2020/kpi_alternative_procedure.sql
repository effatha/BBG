	...
	TRUNCATE TABLE WR.WR_L1_FACT_A_SALES_TRANSACTION_KPI;
	

;WITH CTE_ENVIRO as 
    (
	        SELECT ID_ITEM,CD_COUNTRY_DELIVERY,NUM_YEAR,CD_CURRENCY,sum(AMT_FEE_PER_UNIT) AMT_FEE_PER_UNIT from [L1].[L1_DIM_A_ENVIRO_REST_UNIT_FEES]
	...
	(
      ...
      ,[ID_COMPANY]
      ,[D_CREATED] = DATEADD(YEAR,3,D_CREATED)
	  ,[D_CREATED_ORI] = D_CREATED
      ,[D_SALES_PROCESS]
      ,[DT_CREATED]
      ,[CD_TYPE]
...
,CTE_SALES_L1 AS
	(
			...
			,AMT_TAX_TOTAL_PAYABLE_FC                                           AS TaxTotalPayableForeignCurrency
            ,AMT_TAX_OUTPUT_EUR                                                 AS TaxOutput
			,AMT_TAX_OUTPUT_FC                                                  AS TaxOutputForeignCurrency
			...
			,fact.AMT_TOTAL_NET_PRICE_EUR
			,fact.AMT_TOTAL_NET_PRICE_FC
			,fact.AMT_TAX_REVERSED_EUR
			,fact.AMT_TAX_REVERSED_FC
			,fact.AMT_TOTAL_PRICE_EUR
			,fact.AMT_TOTAL_PRICE_FC
			,fact.CD_PRECEDING_DOCUMENT_NO 
            ,fact.CD_LINKED_DOCUMENT_NO 
			,fact.AMT_NET_SHIPPING_REVENUE_FC
            ,fact.CD_CARRIER
			,fact.VL_REFUND_RATE 					
	        ,fact.VL_RETURN_RATE					
	...
	INSERT INTO WR.WR_L1_FACT_A_SALES_TRANSACTION_KPI 
		(
		 ID_SALES_TRANSACTION
		,CD_SALES_TRANSACTION
		,CD_SOURCE_SYSTEM
	...
	,[D_EFF_TO]																									  as [D_EFF_TO]	
	,[D_EFF_DELETED]																						      as [D_EFF_DELETED]
